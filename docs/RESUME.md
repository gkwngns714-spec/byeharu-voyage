...
| **#83** `osn-pacific-americas-ports` (row 99) | 14 Pacific-Americas harbours + Hobart, 32 goods, migrations **0089** (one furnishing rule), **0090** (harbours/goods/offers, re-derives `port_goods`), **0091** (raster + roadsteads re-emitted, `LAST` pin). | head `fee87f9`: main `3886f42` AND #82 (0088) merged in, row 99, `LAST` = 0091; locally `db:apply` 82/82, `db:proof` 74/74, 0088 + 0091 compose. CI: build + disposable-chain PASS; acceptance pending; **pglite-gate FAILS by the 30-min job ceiling, not by an assert** (the same chain passed the gate in 19 min on a fast runner, run 34811820042) — re-run, or raise `timeout-minutes` in `.github/workflows/migrations-apply-proof.yml` (nobody touched it today). |
...
| **HALF OF THE OWNER'S *"fix both of those"* — RELEASED, do not re-raise** | DEV_LOG's 0082 entry calls itself *"the first of 'fix both'"*, and the second reached no file in this repo. The owner released it on 2026-09-10 — *"row 75 - if there is no issue, forget it"* — so **`OWNER_REQUESTS.md` row 75 is CLOSED by decision**. A cold reader who spots that phrase in the log must NOT ask about it again. Row 74 no longer waits on it. |
...
| ~~**MARKET into PORT fold**~~ | **BUILT 2026-09-11 on branch `osn-quay-board`** as slice 1 of owner row 76 (`docs/QUAY_LEDGER.md`); see the closed row further down. Not merged, not deployed, not driven — rule 2 still applies. |
