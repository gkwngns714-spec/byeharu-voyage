...
| **MARKET into PORT fold** | **The owner's row 73 has all but decided this.** Driven 2026-09-09: at Dublin, MARKET and PORT · Trade draw the SAME eight tiles with the same cells and the same tray — MARKET only adds a port field above it. Once BUY/SELL move onto PORT · Market as row 73 asks, MARKET-the-tab is a second door onto one room. Recommendation: fold, nav 6 cells → 5, keep the port field inside PORT's Trade face. Costed by the row-73 agent; **deliberately NOT built inside that slice** — it is a second concept. |
| **Issue #48** | Harvest 0060's channel research against today's generator. |
| **Owner row 73** | *"they should be located accordingly at different locations"* — a verb's DOORWAY moves to the building whose act it is; the authority (`cmd.issue`, one parser) does not move. Dispatched 2026-09-09 in worktree `bv-verb-homes`, branch `osn-verb-homes`. |
| **Owner row 74** | *"how much i bought the item… the profits of this trade"* — cost basis carried on the cargo, served, average paid per tun; realised profit quoted by the SERVER, never subtracted on the client. Dispatched 2026-09-09 in worktree `bv-trade-basis`, branch `osn-trade-basis`, migration **0081**. |
...
| **MARKET's port field is BROKEN on production** | Found by driving, 2026-09-09, reproduced four times: pick a port from the chips and the field keeps focus, goes deaf to every keystroke, and never reopens — one search per page load. `PortField.tsx`'s `pick()` closes without the `blur()` that `Escape` and `Enter` both do, so `open` goes false while the input keeps focus and `onFocus` — the only thing that reopens it — can never fire again. Handed to the row-73 agent, whose slice owns the file. |
