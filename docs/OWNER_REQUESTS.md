...
| 15 | *"i told you, when i click buy, the tab should be not unfolded. it should stay where it is."* | **DONE — verified** | **Said twice.** Measured: nothing scrolls or jumps (scrollY 0 to 0, BUY card fixed at y=454), so it is NOT a layout-shift bug. The fault is that all SIX verb cards stay expanded after one is chosen, so the goods list sits far below the fold. Reading taken: the verb grid collapses to the chosen verb, so the next step is where the eye already is. |
