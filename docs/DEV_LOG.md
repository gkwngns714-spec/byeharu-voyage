...
## 2026-09-14 — Istanbul on the shore, every harbour on the globe, and the regions painted behind a switch (rows 91, 92, 93 — built on the `osn-map-regions-and-landfall` PR, not merged)
...
### Row 91 — measured first: 79 harbours in the water the chart draws
...
### Row 93 — every harbour, as a dot, and the countries were never culled
...
### Row 92 — the regions, measured before painted
...
* `tests/map.regions.spec.ts` — **11 passed** (7 pure, 4 browser: dark and light × 390×844 and 1440×900) — off by default, `aria-pressed=false`, 0 tint paths, 0 region names; on: 25 water paths, 24 land paths, `Iberia` named at the opening frame, `map-sea`/`map-coastline`/`map-coast` the same computed colours; off again `toEqual` the first sheet (164 elements at 390, 122 at 1440; 216 and 174 with the filter on); kept across a reload. Frame cost of a zoom step (twelve wheel steps,
  two frames each, 1440×900): measured IN-PAGE (`zoomStepMs` in `tests/appReady.fixture.ts` — twelve synthetic wheel steps, two frames each; the first apparatus timed Playwright's own round trips and read ~150 ms a step of driver latency) at the opening frame, one worker: **1440×900 dark OFF 195.0 · ON 182.7 ms; light OFF 206.6 · ON 204.5 ms; 390×844 dark OFF 76.2 · ON 60.2 ms; light OFF 73.2 · ON 78.4 ms** — on against off is inside the noise, and the tint is never a slower frame. The ABSOLUTE figures are this machine's today, not the chart's: `main`'s own build, same apparatus, same glass, read **184.6 · 241.9 ms** a step against this branch's **108.2 · 109.9 ms** in the next minute, with five other agents' builds and suites running (12–21 node processes); row 90's 31–44 ms was an idle machine.
* `tests/map.marks.spec.ts` — **9 passed** (5 pure, 4 browser) — at the world view 224 harbour marks in the DOM, 35 full and 189 dots, at both glasses in both schemes (opening frame 25 harbours at 390 — 15 full, 10 dots — and 12 at 1440 — 6 and 6); the whole body path (78.9 KB) at the world view as at the opening; a dot (Acapulco) wheel-zoomed in on wears `data-port-mark="full"`. Frame cost at the world view with all 224 marks on
  the sheet: in-page, one worker, all 224 marks on the sheet: **1440×900 dark 122.7 ms · light 166.3 ms; 390×844 dark 31.0 ms · light 66.9 ms** — under the same machine's opening-frame figures above, so the dots cost nothing the eye can measure.
* The guards: `npx tsc -b` clean · `npx eslint .` clean · `npm run build` ✓ (the first, with the world image rebuilt: 13 m 58 s; the second 1.18 s) · the full brief suite (`tests/map.*.spec.ts chart.ink layout wide.layout words duplication`, 204 tests, 2 workers, `localhost:4191`): **203 passed, 1 failed, 0 skipped** in 10.5 min — the one red was `map.sendfleet`'s positive control, which took the first `g[data-port-code]` after Cádiz as "any other harbour" and got Agadir's DOT, which has no path to aim at; the drive now aims at `data-port-mark="full"` (pin moved deliberately, dated in the file) and `map.sendfleet` re-ran **9 passed** on the same build. The earlier full run against the same code had caught the real finding this section records above — a tap on *Cadiz* opening Sanlúcar with dots in the hit test — which is why dots are pictures now.
...
than fixed twice — the same one D44 reported.
