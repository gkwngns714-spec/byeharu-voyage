...
| `Screen.tsx` + `screenLayout.ts` | 23 + 29 | **CARRIED, trimmed, then re-earned.** `Screen` owns the scroll and the `space-y-4` rhythm and is used by every tab. `screenSplitClass()` / `screenRailClass()` — the desktop two-rail split — were **deleted on 2026-08-22 with no caller ever written**: the app is mobile-first and no screen adopted them in four months. **Later the same day the split came back**, in the shape of the screen that finally needed it, as `splitClass()` / `splitMainClass()` / `splitRailClass()` — the owner asked for the goods list and the fleet's own state side by side while buying, and COMMAND's `OrderComposer` is the first and only caller. The flex-not-grid reasoning, the `md` breakpoint and the "no action inside a sticky rail" rule are written into `screenLayout.ts` itself now, against a layout that exists, rather than kept warm in this row. |
