...
| `Screen.tsx` + `screenLayout.ts` | 23 + 29 | **CARRIED, then trimmed.** `Screen` owns the scroll and the `space-y-4` rhythm and is used by every tab. `screenSplitClass()` / `screenRailClass()` — the desktop two-rail split — were **deleted on 2026-08-22 with no caller ever written**: the app is mobile-first and no screen adopted them in four months. The flex-not-grid reasoning was sound and is recorded here rather than kept as dead code; bring it back from this row if a desktop layout is ever built. |
