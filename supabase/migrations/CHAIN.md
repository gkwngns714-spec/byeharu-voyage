...
* **Supabase's own roles.** 0001's shims create `anon`/`authenticated` when absent, so locally the
  lockdown is proven against roles this chain made. Supabase creates them itself, *with* a default
  `GRANT ALL` — the exact drift that aborted the predecessor's deploy. Only the disposable-Supabase
  job in `.github/workflows/migrations-apply-proof.yml` proves the revoke lands against those.
