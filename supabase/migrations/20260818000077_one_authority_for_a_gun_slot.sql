-- ===============================================================================================
-- 0077 - ONE AUTHORITY FOR A GUN SLOT
-- ===============================================================================================
--
...
begin
  while v_i < array_length(p_edits, 1) loop
    ...
    if v_n <> 1 then
      raise exception '0077 slice: hunk % of % occurs % time(s) in %, expected exactly 1 - the deployed body is not what this migration was generated against.',
...
select pg_temp.recut('cmd.do_fit(uuid, jsonb)'::regprocedure, false,
  ...
  $f1$  -- 0077: ONE TABLE FOR EVERY KIND OF SLOT, weapons included. This was a branch on
...
select pg_temp.recut('world.fleets()'::regprocedure, false,
  ...
  $w1$                 -- 0077: class_slots alone. An override used to be appended here that replaced a
...
declare
  ...
  v_res    jsonb;
  v_grants int;
  c_uid constant uuid := '00000000-0077-4000-8000-000000000001';
begin
  ...
  if v_bad <> 0 then
    raise exception '0077 self-assert FAIL: % class(es) have no weapon slot in class_slots', v_bad;
  end if;
  ...
  if position('c.guns' in pg_get_functiondef('cmd.do_fit(uuid, jsonb)'::regprocedure)) <> 0 then
    raise exception '0077 self-assert FAIL: cmd.do_fit still reads ship_classes.guns for a slot';
  end if;
  ...
  if position('''weapon'', c.guns' in pg_get_functiondef('world.fleets()'::regprocedure)) <> 0 then
    raise exception '0077 self-assert FAIL: world.fleets still overrides the weapon slot with guns';
  end if;
  ...
  if position('guns' in pg_get_functiondef('voyage.settle(uuid, timestamptz)'::regprocedure)) = 0 then
    raise exception '0077 self-assert FAIL: the escort score stopped reading guns';
  end if;
  ...
  if (select c.code from public.ships s join public.ship_classes c on c.id = s.class_id
    ...
    raise exception '0077 self-assert FAIL: the founding hull is not a barca, so this probe proves nothing';
  end if;
  ...
  if not (v_res->>'ok')::boolean then
    raise exception '0077 self-assert FAIL: a barca still cannot mount a weapon: %', v_res->'order';
  end if;
  if not exists (select 1 from public.ship_fittings where ship_id = v_ship and item_code = 'boarding-gear') then
    raise exception '0077 self-assert FAIL: FIT said yes and mounted nothing';
  end if;
  ...
  if (v_res->>'ok')::boolean then
    raise exception '0077 self-assert FAIL: a barca mounted TWO weapons in her one slot';
  end if;
  if v_res->'refusal'->>'code' <> 'E_SLOTS_FULL' then
    raise exception '0077 self-assert FAIL: her full weapon slot refused with % rather than E_SLOTS_FULL',
      v_res->'refusal'->>'code';
  end if;
