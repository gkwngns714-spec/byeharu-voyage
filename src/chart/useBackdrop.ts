...
export function useBackdrop(): BackdropState {
  const [state, setState] = useState<BackdropState>({ coast: null, seas: NO_SEAS, error: null })

  useEffect(() => {
    const controller = new AbortController()
    import('./backdrop')
      .then(({ loadBackdrop }) => loadBackdrop(controller.signal))
      .then((data) => setState({ coast: data.coast, seas: data.seas, error: null }))
      .catch((cause: unknown) => {
        // An abort is this effect cleaning up (React 19 StrictMode mounts twice in development),
        // not a failure to report.
        if (controller.signal.aborted) return
        setState({ coast: null, seas: NO_SEAS, error: cause instanceof Error ? cause.message : 'chart unavailable' })
      })
