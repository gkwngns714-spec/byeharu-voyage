...
// 0062's own word — is appended to the caption the row already carries (`· 40 t aboard · native`),
// in the caption voice. Not a coloured tag: docs/UI_DIRECTION.md §4.4 gives rarity its hues as a
// 10-px MARK only and never as text colour, and a second badge vocabulary beside it would be a
// second one. A server predating 0084 sends no flag and the row draws nothing — never a guess from
// origin_regions on this side of the wire. The rarity mark IS drawn, mark only, when the tier is
// served — never as a text tag.
        ...
        <span className="flex items-center gap-1.5 text-t-body">
          ...
          {(aboard > 0 || good.native === true) && (
            <span className="whitespace-nowrap text-t-caption text-ink-faint">
              {[aboard > 0 ? `${formatTuns(aboard)} aboard` : null, good.native === true ? 'native' : null]
                .filter(Boolean)
                .map((w) => `· ${w}`)
                .join(' ')}
            </span>
