...
/**
 * WHAT A LINE DOES TO THE PURSE, SIGNED — a sold line is a credit, a bought line a debit. The server
 * serves `side` and an unsigned `total`; the sign is presentation, and it was spelt twice (the
 * basket face and the receipt face, PR #59 review SHOULD 4b), so it is spelt here, once, at the
 * boundary both faces read through.
 */
export function lineDelta(line: Pick<ManifestReceiptLine, 'side' | 'total'>): number {
