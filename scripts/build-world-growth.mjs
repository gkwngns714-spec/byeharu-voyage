...
//   * SEA_PLACE port rows — 0036 owns them (generated from
...
//
// WHAT A NEW HARBOUR NEEDS BESIDES ITS ROW (learned 2026-09-14, the first growth to add a harbour
// since the leg graph retired; 0041 predates all of it):
//   * its MARKET ROWS carry demand (0066): the port_goods insert below asks world.demand_for(),
//     the one derivation, for every inserted pair. A row left at the column default (1.000) would
//     price without an appetite where every other row of the world has one.
//   * its BUILDINGS (0067/0068/0070/0072 seeded them once, inline, over the harbours of their day):
//     the migration calls public.furnish_harbours() — 0089's ONE rule — after inserting harbours.
//     The rule is not restated here; the assert only checks that no harbour lacks a row it names.
//   * its ROADSTEAD and its SAILED DISTANCES (sea_reaches, 0046/0076/0085): NOT this file's.
//     scripts/build-sea-migration.mjs reads the ports out of the APPLIED chain, so it can only be
//     cut AFTER this migration exists — which is why self-assert (g) below asserts, at this file's
//     own position, that the ONLY ports without a full reach row are exactly the harbours this
//     file inserted, and scripts/db/proofs/08 (REACH_TOTAL_AT_EVERY_PORT) holds the finished chain
//     to "every port carries a full reach row". A growth pushed without its sea migration is red in
//     db:proof, not merely in prose.
//   Each of these is detected from the baseline (does the column / function exist?) so the
//   generator still emits a correct file against any baseline it is pointed at.
// ═══════════════════════════════════════════════════════════════════════════════════════════════
...
const hasColumn = async (table, column) => (await db.query(
  `select 1 from information_schema.columns where table_schema = 'public' and table_name = $1 and column_name = $2`,
  [table, column],
)).rows.length > 0
const hasOrigin = await hasColumn('goods', 'origin_regions')   // 0062
const hasIndustry = await hasColumn('goods', 'industry')       // 0064
const hasDemand = await hasColumn('port_goods', 'demand')      // 0066
const hasFurnish = (await db.query(
  `select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'furnish_harbours'`,
)).rows.length > 0                                             // 0089
...
base.goods = (await db.query(`
  select code, name, base_value::float8 base_value, bulk::float8 bulk,
         ...
         ${hasOrigin ? ', origin_regions, entrepot_ports' : ''}
         ${hasIndustry ? ', industry' : ''}
    from public.goods order by code`)).rows
// (legs) RETIRED with migration 0049: public.legs is dropped; the raster (0046) and the reach
// table are the water authority, regenerated by scripts/build-sea-migration.mjs when ports move.
...
const changedGoods = want.goods.filter((g) => {
  ...
  return b.name !== g.name || Math.abs(b.base_value - g.base_value) > 1e-9 || Math.abs(b.bulk - g.bulk) > 1e-9
    ...
    || JSON.stringify(pgArr(b.culture_mask)) !== JSON.stringify(g.culture_mask)
    // 0062 gave a good its geography and 0064 its industry flag; a growth that changed a good's
    // origin (a region added to where it grows) and did not carry it would fail world-guard.
    || (hasOrigin && (JSON.stringify(pgArr(b.origin_regions)) !== JSON.stringify(g.origin_regions)
                      || JSON.stringify(pgArr(b.entrepot_ports)) !== JSON.stringify(g.entrepot_ports)))
    || (hasIndustry && Boolean(b.industry) !== Boolean(g.industry))
})
...
const anything = newPorts.length + changedPorts.length + newGoods.length + changedGoods.length
  + specAdd.length + specDel.length
if (anything === 0) {
  console.log('the applied world already equals data/*.json — nothing to emit, no file written')
  process.exit(0)
}

console.log(`delta: +${newPorts.length} ports (~${changedPorts.length} updated) · +${newGoods.length} goods (~${changedGoods.length} updated) · offers +${specAdd.length}/-${specDel.length}`)
...
--        ${want.specialtyPairs.length} port offers — landed as a DELTA on whatever world this
...
--   ~${String(changedPorts.length).padEnd(4)} harbour rows updated (dev_* follow the roster)
--   +${String(newGoods.length).padEnd(4)} goods; ~${changedGoods.length} changed (name, band, geography — codes never change)
--   (sea legs: RETIRED with 0049 — a new or moved port needs scripts/build-sea-migration.mjs
--    to cut a NEW raster/reaches migration AFTER this growth, or the new harbour has no roadstead
--    and no sailed distances; self-assert (g) names the harbours awaiting it and proof 08 refuses
--    a finished chain that never seeded them)
--   +${specAdd.length}/-${specDel.length} port offers, then port_goods re-derived for EVERY (harbour, good) pair${hasDemand ? `
--   (each NEW pair asking world.demand_for() for its city's appetite, 0066)` : ''}${hasFurnish ? `
--   the ${newPorts.length} new harbour(s) furnished by public.furnish_harbours(), 0089's one rule` : ''}
...
if (changedGoods.length) {
  w(`-- ── 2b. ${changedGoods.length} goods whose data changed (name/category/price band/geography); codes never move ──`)
  w(`update public.goods g
   set name = v.name, base_value = v.base_value, bulk = v.bulk,
       perishable_pct_day = v.perish, category = v.category, culture_mask = v.mask::text[]${hasOrigin ? `,
       origin_regions = v.origin::text[], entrepot_ports = v.entrepot::text[]` : ''}${hasIndustry ? `,
       industry = v.industry` : ''}
  from (values`)
  w(changedGoods.map((g) =>
    `  (${q(g.code)}, ${q(g.name)}, ${g.base_value.toFixed(2)}::numeric, ${g.bulk.toFixed(3)}::numeric, ${g.perishable_pct_day.toFixed(3)}::numeric, ${q(g.category)}, ${q('{' + g.culture_mask.join(',') + '}')}, ${q('{' + g.origin_regions.join(',') + '}')}, ${q('{' + g.entrepot_ports.join(',') + '}')}, ${g.industry ? 'true' : 'false'})`,
  ).join(',\n'))
  w(`  ) as v(code, name, base_value, bulk, perish, category, mask, origin, entrepot, industry)
 where g.code = v.code
   and (g.name, g.base_value, g.bulk, g.perishable_pct_day, g.category, g.culture_mask${hasOrigin ? ', g.origin_regions, g.entrepot_ports' : ''}${hasIndustry ? ', g.industry' : ''})
       is distinct from (v.name, v.base_value, v.bulk, v.perish, v.category, v.mask::text[]${hasOrigin ? ', v.origin::text[], v.entrepot::text[]' : ''}${hasIndustry ? ', v.industry' : ''});
`)
}
...
// (5. legs) RETIRED with migration 0049 — see the header note.
...
insert into public.port_goods (port_id, good_id, affinity, stock, stock_target, production_rate${hasDemand ? ', demand' : ''})
select p.id,
       g.id,
       aff.affinity,
       ...
       case when aff.affinity < 0.80
            ...
            else 0 end${hasDemand ? `,
       -- 0066: a NEW pair learns what its city wants through the one derivation; an existing pair
       -- keeps its demand (the region and the category did not move, so neither did it)
       world.demand_for(p.id, g.id)` : ''}
  from public.ports p
...
// 7. the derived rows that follow a harbour
if (hasFurnish) {
  w(`-- ── 7. A NEW HARBOUR IS FURNISHED BY 0089's ONE RULE ─────────────────────────────────────────
-- public.furnish_harbours() inserts, for every harbour, each building row public.harbour_buildings
-- names that the city does not yet keep — and nothing else: a kept row is the game's. On a world
-- with no new harbour it writes 0 rows; the rows it writes here are exactly the new harbours'.
select public.furnish_harbours() as buildings_furnished;
`)
}
...
create temporary table want_goods_${shortNo} (
  ...
  origin text[], entrepot text[], industry boolean
);
insert into want_goods_${shortNo} values`)
w(want.goods.map((g) =>
  `  (${q(g.code)}, ${q(g.name)}, ${g.base_value.toFixed(2)}, ${g.bulk.toFixed(3)}, ${g.perishable_pct_day.toFixed(3)}, ${q(g.category)}, ${q('{' + g.culture_mask.join(',') + '}')}, ${q('{' + g.origin_regions.join(',') + '}')}, ${q('{' + g.entrepot_ports.join(',') + '}')}, ${g.industry ? 'true' : 'false'})`,
).join(',\n') + ';')
w(`
...
begin
  ...
  select count(*) into v_n
   ...
   where (g.name, g.base_value, g.bulk, g.perishable_pct_day, g.category, g.culture_mask${hasOrigin ? ', g.origin_regions, g.entrepot_ports' : ''}${hasIndustry ? ', g.industry' : ''})
         is distinct from (wg.name, wg.base_value, wg.bulk, wg.perish, wg.category, wg.mask${hasOrigin ? ', wg.origin, wg.entrepot' : ''}${hasIndustry ? ', wg.industry' : ''});
  if v_n <> 0 then
    select string_agg(wg.code, ', ') into v_list from want_goods_${shortNo} wg
      join public.goods g on g.code = wg.code
     where (g.name, g.base_value, g.bulk, g.perishable_pct_day, g.category, g.culture_mask${hasOrigin ? ', g.origin_regions, g.entrepot_ports' : ''}${hasIndustry ? ', g.industry' : ''})
           is distinct from (wg.name, wg.base_value, wg.bulk, wg.perish, wg.category, wg.mask${hasOrigin ? ', wg.origin, wg.entrepot' : ''}${hasIndustry ? ', wg.industry' : ''});
    raise exception '${shortNo} self-assert FAIL: % good row(s) disagree with the data column-for-column (geography included): %', v_n, v_list;
  end if;
...
${hasOrigin ? `  --     0062's law: every offer is NATIVE (the port's region is among the good's origin_regions) or
  --     a NAMED ENTREPOT (the port's code is in the good's entrepot_ports). No third case.
  select count(*) into v_n
  ...
  if v_n <> 0 then
    select string_agg(p.code || '·' || g.code, ', ') into v_list
      from public.port_specialties s
      join public.ports p on p.id = s.port_id
      join public.regions r on r.id = p.region_id
      join public.goods g on g.id = s.good_id
     where not (r.code = any(g.origin_regions)) and not (p.code = any(g.entrepot_ports));
    raise exception '${shortNo} self-assert FAIL: % offer(s) are neither native nor a named entrepot (0062): %', v_n, v_list;
  end if;
  --     ... and every good is produced somewhere and buyable at a producer (0062 (d)).
  select count(*) into v_n from public.goods g
   where cardinality(g.origin_regions) = 0
      or not exists (select 1 from public.port_specialties s join public.ports p on p.id = s.port_id
                       join public.regions r on r.id = p.region_id
                      where s.good_id = g.id and r.code = any(g.origin_regions));
  if v_n <> 0 then
    raise exception '${shortNo} self-assert FAIL: % good(s) have no origin or are buyable at no producer (0062)', v_n;
  end if;
  --     0065's law: no good sits in four cities.
  select count(*) into v_n from (
    select good_id from public.port_specialties group by good_id having count(*) > 3) x;
  if v_n <> 0 then
    select string_agg(g.code || '×' || x.c, ', ') into v_list from (
      select good_id, count(*) as c from public.port_specialties group by good_id having count(*) > 3) x
      join public.goods g on g.id = x.good_id;
    raise exception '${shortNo} self-assert FAIL: % good(s) sit in four or more cities (0065): %', v_n, v_list;
  end if;
` : ''}${hasFurnish ? `  --     0089's rule: no harbour lacks a building the rule names (tiers are the game's, so only
  --     the KIND is asserted; 0089 itself proves the tiers on the world it found).
  select count(*) into v_n
    from public.ports p
    cross join lateral public.harbour_buildings(p.id) b
   where p.kind = 'HARBOUR'
     and not exists (select 1 from public.port_buildings pb where pb.port_id = p.id and pb.kind = b.kind);
  if v_n <> 0 then
    raise exception '${shortNo} self-assert FAIL: % (harbour, building) row(s) the one rule names are missing — furnish_harbours did not run or did not reach them', v_n;
  end if;
` : ''}
  ---------------------------------------------------------------------------------------------
  -- (g) THE WATER, AT THIS FILE'S OWN POSITION. sea_reaches (the 0046 family) is cut by
  --     scripts/build-sea-migration.mjs from the APPLIED chain, so a harbour inserted HERE cannot
  --     have a reach row until the sea migration that follows this one. What CAN be asserted here,
  --     and is: every port that existed before this file still carries a reach row over the
  --     pre-image's ports, and the ONLY ports lacking a full row are EXACTLY the harbours this
  --     file inserted — named. A pre-existing port that lost its water, or a stray port from
  --     nowhere, is refused. The finished chain is held to "every port carries a full reach row"
  --     by scripts/db/proofs/08 (REACH_TOTAL_AT_EVERY_PORT), so a growth pushed without its sea
  --     migration is red in db:proof.
  ---------------------------------------------------------------------------------------------
  select count(*) into v_ports_all from public.ports;
  select count(*) into v_reachable from public.sea_reaches sr
   where (select count(*) from jsonb_object_keys(sr.reaches)) = v_ports_all - 1 - ${newPorts.length};
  if v_reachable <> v_ports_all - ${newPorts.length} then
    raise exception '${shortNo} self-assert FAIL: % of the % pre-existing ports carry a reach row over the pre-existing world — the water this file found is not whole', v_reachable, v_ports_all - ${newPorts.length};
  end if;
  select string_agg(p.code, ', ' order by p.code) into v_list
    from public.ports p
   where not exists (select 1 from public.sea_reaches sr where sr.port_id = p.id);
  if coalesce(v_list, '') is distinct from ${q(newPorts.map((p) => p.code).sort().join(', '))} then
    raise exception '${shortNo} self-assert FAIL: the ports without a reach row are [%], expected exactly the harbours this file inserted [${newPorts.map((p) => p.code).sort().join(', ')}] — the sea migration that follows must seed them', coalesce(v_list, '');
  end if;
...
${hasDemand ? `  --     0066: every market row of a harbour this file inserted carries the demand the one
  --     derivation answers (the default 1.000 would be a city with no appetite among 25 that have).
  select count(*) into v_n
    from public.port_goods pg
    join public.ports p on p.id = pg.port_id
   where p.code in (${newPorts.map((p) => q(p.code)).join(', ') || q('')})
     and pg.demand is distinct from world.demand_for(pg.port_id, pg.good_id);
  if v_n <> 0 then
    raise exception '${shortNo} self-assert FAIL: % market row(s) at the new harbours carry a demand the one derivation does not answer', v_n;
  end if;
` : ''}
  ---------------------------------------------------------------------------------------------
  ...
  raise notice '${shortNo} self-assert ok: THE WORLD EQUALS THE DATA — % harbours (+% over the world this file found) and % goods (+%) with every column equal to data/*.json${hasOrigin ? ' (origin and entrepot columns included)' : ''}; the offer set equals every roster as a set (positive control: a planted bogus offer WAS seen, then removed); dev_* restate every roster, all % harbours satisfy the roster count law (capital 10 / mid 4-8 / small 4)${hasOrigin ? ', every offer is native or a named entrepot, every good is buyable at a producer, no good sits in four cities' : ''}${hasFurnish ? ', no harbour lacks a building the one rule names' : ''}; all % pre-existing ports carry a reach row and the ${newPorts.length} inserted harbour(s) await the sea migration that follows; % market rows = harbours × goods (% more than the % this file found), every affinity re-derivable from the one formula, 0005''s stock arithmetic holding on every row${hasDemand ? ', every new row carrying its city\x27\x27s demand' : ''}; knobs at ${Object.entries(knobLits).map(([k, v]) => `${k.replace('affinity_', '')} ${v}`).join(', ')}; 0 client write grants, 0 client-executable writers, 0 read-wall gaps',
    v_harbours, v_harbours - pre.harbours, v_goods, v_goods - pre.goods,
    v_harbours, v_ports_all - ${newPorts.length},
