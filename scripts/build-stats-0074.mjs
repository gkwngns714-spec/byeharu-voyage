// Emits migration 0074 from data/ship_stats.json and data/fittings.json.
import fs from 'node:fs'

const stats = JSON.parse(fs.readFileSync('data/ship_stats.json', 'utf8'))
const fittings = JSON.parse(fs.readFileSync('data/fittings.json', 'utf8')).fittings

const STATS = ['hold', 'speed', 'handling', 'draft', 'durability', 'armour', 'attack', 'accuracy', 'reach', 'sighting']
const SLOTS = ['rig', 'weapon', 'ground-tackle', 'steering', 'hull', 'lookout', 'flourish']

const q = (s) => `'${String(s).replace(/'/g, "''")}'`

for (const f of fittings) {
  if (!SLOTS.includes(f.slot)) throw new Error(`${f.code}: unknown slot "${f.slot}"`)
  for (const e of f.effects ?? []) {
    if (!STATS.includes(e.stat)) throw new Error(`${f.code}: unknown stat "${e.stat}"`)
    if (e.delta === 0) throw new Error(`${f.code}: an effect of zero is not an effect`)
  }
  // DESIGN 1.3, checked where the data is written rather than only where it lands.
  const eff = f.effects ?? []
  const up = eff.some((e) => e.delta > 0)
  const down = eff.some((e) => e.delta < 0)
  if (f.code !== 'figurehead' && !(up && down)) {
    throw new Error(`${f.code}: every fitting buys one stat and SPENDS another (DESIGN 1.3)`)
  }
  if (f.code === 'figurehead' && eff.length !== 0) {
    throw new Error('the figurehead costs no stat, deliberately (DESIGN 1.4)')
  }
}

const classRows = stats.classes
  .map((c) => `  (${q(c.class)}, ${c.handling}, ${c.armour}, ${c.attack}, ${c.accuracy}, ${c.reach}, ${c.sighting}, ${q(c.note)})`)
  .join(',\n')

const slotRows = stats.slots
  .flatMap((s) => SLOTS.filter((k) => s[k] > 0).map((k) => `  (${q(s.class)}, ${q(k)}, ${s[k]})`))
  .join(',\n')

const cabinRows = stats.slots.map((s) => `  (${q(s.class)}, ${s.cabins})`).join(',\n')

const effectRows = fittings
  .flatMap((f) => (f.effects ?? []).map((e) => `  (${q(f.code)}, ${q(e.stat)}, ${e.delta})`))
  .join(',\n')
...
fs.writeFileSync('supabase/migrations/20260818000074_a_fitting_changes_the_ship.sql', SQL)
console.log('wrote 0074:', stats.classes.length, 'classes,',
  stats.slots.reduce((n, s) => n + SLOTS.filter((k) => s[k] > 0).length, 0), 'slot rows,',
  fittings.reduce((n, f) => n + (f.effects ?? []).length, 0), 'effect rows')
