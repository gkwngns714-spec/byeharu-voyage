...
w(`--   voyage.water_snap_nm(numeric, numeric), created at 0047:199. This file SUPERSEDES it, and`)
w(`--   the supersede is a fold, not a change of rule: the body is moved down into`)
w(`--   voyage.water_roadstead(lat, lon) UNCHANGED but for keeping the point it already computed`)
w(`--   and threw away (0047:225-228), and water_snap_nm becomes one line over it. The SIGNATURE`)
w(`--   does not move, no function is dropped, and its three existing callers (0047:594, 0047:612,`)
w(`--   0047:1186-1189) are untouched and read the same numbers as before — so the supersede is a`)
w(`--   no-op for every caller that existed before this file. What it buys is one body answering`)
w(`--   "where is the water nearest this point, and how far", which is what lets self-assert (e)`)
w(`--   re-ask PostgreSQL the question the Node generator answered, for every row.`)
w(`--   cmd.do_sail, voyage.assert_paths_water and world.snapshot() are RE-CUT, not superseded:`)
w(`--   each is sliced hunk-by-hunk out of its DEPLOYED body through pg_get_functiondef, so the`)
w(`--   rest of every one of those bodies is carried across byte-identical.`)
w(`--`)
