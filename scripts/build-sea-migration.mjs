// ═══════════════════════════════════════════════════════════════════════════════════════════════
// build-sea-migration.mjs — writes THE NAVIGABLE SEA, AS DATA. Currently migration 0091.
//
...
const MIGRATION = '20260818000091_the_water_reaches_the_new_harbours.sql'
...
// ── WHAT IS NEW: every place the applied table has never heard of ─────────────────────────────
// A growth migration (0041, 0090, …) inserts harbours; this generator then reads them out of the
// applied chain and gives each one its roadstead and its sailed distances. They are named here,
// printed into the header, and asserted BY NAME below, with the snap this run measured.
const newPlaces = ports.filter((p) => !appliedReaches.has(p.code))
console.log(`  ${newPlaces.length} place(s) are new to sea_reaches (no applied row):`)
for (const p of newPlaces) {
  const rd = roads.get(p.code)
  console.log(`    ${p.code.padEnd(4)} ${p.name.padEnd(22)} roadstead (${rd.lat}, ${rd.lon}) ${rd.nm.toFixed(2).padStart(6)} nm${rd.channel ? ` on ${rd.channel}` : ''}`)
}
...
w(`-- ═══════════════════════════════════════════════════════════════════════════════════════════════`)
w(`-- ${SHORT} — THE WATER REACHES THE NEW HARBOURS`)
w(`--        Every place the world grew since the last sea migration gets its roadstead and its`)
w(`--        sailed distance to every other place — and two channels open the water the map is`)
w(`--        too coarse to draw: the Golden Gate and the Guayas.`)
w(`-- ═══════════════════════════════════════════════════════════════════════════════════════════════`)
w(`--`)
...
w(`--`)
w(`-- ── THE OWNER, VERBATIM (docs/OWNER_REQUESTS.md, row 91, 2026-09-14) ─────────────────────────────`)
w(`--   "I see no cities on the left side of america, the number of cities are weird. Check"`)
w(`--`)
w(`-- ── WHAT THIS RUN FOUND ────────────────────────────────────────────────────────────────────────`)
w(`--   ${newPlaces.length} place(s) exist in public.ports with NO row in public.sea_reaches — the harbours the`)
w(`--   growth migration before this one inserted (${newPlaces.map((p) => p.code).join(' ')}). Until this file`)
w(`--   they have no roadstead: world.snapshot serves them null, cmd.do_sail cannot sail to or from`)
w(`--   them, and world.trade_routes cannot price a passage that touches them. The generator reads`)
w(`--   the ports out of the APPLIED chain, which is why it is cut after the growth and not with it`)
w(`--   (build-world-growth.mjs self-assert (g) names them as awaiting this file; proof 08 holds the`)
w(`--   finished chain to every port having its water).`)
w(`--`)
w(`--   TWO CHANNELS OPENED (scripts/sea-grid.mjs CHANNELS, each declaring the cells of dry land it`)
w(`--   turns into sea, refused by the generator if the number is wrong):`)
w(`--   * golden-gate — Natural Earth 1:110m draws no San Francisco Bay at all (three vertices on`)
w(`--     the outer coast), so at 0.25° the bay was land and San Francisco's nearest water lay`)
w(`--     22.42 nm out in the Pacific off Point Reyes. The Gate is a mile wide; the bay behind it`)
w(`--     is the largest harbour on the coast. 3 cells of land opened.`)
w(`--   * guayas — Guayaquil stands 40 nm up the Guayas; the polygon keeps the Gulf of Guayaquil`)
w(`--     but the river and the Puná channel scan-fill as land, so the quay snapped 29.75 nm to the`)
w(`--     gulf. The yard that built the Armada del Mar del Sur floated its galleons down this`)
w(`--     water. 3 cells of land opened.`)
w(`--   The carve opens water 0040's membership raster never saw; each such cell is healed to the`)
w(`--   nearest named sea BY WATER (build-sea-raster.mjs's own rule) and asserted by name below.`)
w(`--`)
w(`-- ── THE NEW PLACES, AS SEEDED (roadstead; snap in nm; the channel under it, if any) ────────────`)
for (const p of newPlaces) {
  const rd = roads.get(p.code)
  w(`--   ${p.code} ${p.name.padEnd(22)} (${rd.lat}, ${rd.lon}) ${rd.nm.toFixed(2).padStart(6)}${rd.channel ? `  on ${rd.channel}` : ''}`)
}
w(`--`)
...
w(`--`)
w(`-- ── WHAT THIS DOES ─────────────────────────────────────────────────────────────────────────────`)
w(`--   1. public.sea_raster is rewritten: the grid with the two new channels carved (+${opened} cells opened,`)
w(`--      -${closed} closed against the applied raster), and public.sea_cells (0040) patched for the`)
w(`--      ${healed.length} newly opened cell(s), each joining its nearest named sea by water.`)
w(`--   2. voyage.channels — the ${CHANNELS.length} authored channels, replaced whole: id, name, the polyline, the`)
w(`--      ${carvedBy.size} cells of dry land the carve opened, and opens_land CHECKed equal to that list's length.`)
w(`--   3. voyage.channel_foot and voyage.water_roadstead — the 0085 rule, re-emitted whole because`)
w(`--      this generator is their one source (a later sea migration carries the current rule rather`)
w(`--      than trusting an earlier file did). SUPERSEDES 0085's bodies with identical ones: the`)
w(`--      drop-and-create of water_roadstead is 0085's own shape, revoke re-issued.`)
w(`--   4. public.sea_reaches is replaced WHOLE — the ${newPlaces.length} new place(s) gain their row, and every`)
w(`--      passage is re-measured roadstead to roadstead over the new water (the 0052:92-94`)
w(`--      delete-and-insert, exactly). ${moved.length} existing roadstead(s) move.`)
w(`--`)
w(`-- ── AND WHAT IT DELIBERATELY DOES NOT ──────────────────────────────────────────────────────────`)
w(`--   * It does not change the roadstead RULE (0085's three arms stand) and it does not move the`)
w(`--     roadstead of any place whose water did not change — ${ports.length - moved.length - newPlaces.length} of ${ports.length}.`)
...
w(`--   * It moves no price knob, re-cuts no mover, touches no player row. Beyond the two channels`)
w(`--     named above it opens and closes no water.`)
if (!waterMoved) {
...
w(`--   THE LAND DATA, asked of every off-quay roadstead the chain serves today: ${landRed.length} stand inside`)
w(`--   a coastline polygon of data/world-110m.json — ${landRed.join(' ')} — (0085 put the river ports ON their channels, and a river is inside its coast; a count here is only RED if one of them is off a channel).`)
w(`--   Asked of what this file seeds: ${landGreen.length} stand inside a coastline polygon without a channel`)
...
w(`--   (a place seeded at snap 0 — its own quay — that now carries a roadstead off it is a river`)
w(`--   port the rule reached; Bristol's quay IS a vertex of the Severn channel, so hers stays 0 nm)`)
w(`--`)
...
w(`  -- (u) THE NEW PLACES, BY NAME. Each place this run found without an applied reach row now`)
w(`  --     carries the roadstead this run measured (0.01 nm is snap_nm's own rounding), on the`)
w(`  --     channel this run put it on, and reaches every other place at a sailed distance no shorter`)
w(`  --     than the great circle between the two roadsteads.`)
for (const p of newPlaces) {
  const rd = roads.get(p.code)
  w(`  select sr.snap_nm, wr.channel into v_snap, v_ref`)
  w(`    from public.sea_reaches sr join public.ports p on p.id = sr.port_id`)
  w(`   cross join lateral voyage.water_roadstead(p.lat, p.lon) wr where sr.code = ${q(p.code)};`)
  w(`  if v_snap is null or abs(v_snap - ${rd.nm.toFixed(2)}) > 0.01 or v_ref is distinct from ${rd.channel === null ? 'null' : q(rd.channel)} then`)
  w(`    raise exception '${SHORT} self-assert FAIL: ${p.name} is % nm off the quay on [%]; this run measured ${rd.nm.toFixed(2)} nm on ${rd.channel ?? 'no channel'}', v_snap, coalesce(v_ref, 'no channel');`)
  w(`  end if;`)
}
w(`  select count(*) into v_bad`)
w(`    from public.sea_reaches a`)
w(`    join public.sea_reaches b on b.code <> a.code`)
w(`   where a.code in (${newPlaces.map((p) => q(p.code)).join(', ')})`)
w(`     and ((a.reaches->>b.code) is null`)
w(`          or (a.reaches->>b.code)::numeric < voyage.gc_distance_nm(a.roadstead_lat::float8, a.roadstead_lon::float8, b.roadstead_lat::float8, b.roadstead_lon::float8) - 0.1);`)
w(`  if v_bad <> 0 then`)
w(`    raise exception '${SHORT} self-assert FAIL: % pair(s) from a new place are missing or shorter than the great circle', v_bad;`)
w(`  end if;`)
w(`  -- and the Pacific coast is one water: the northernmost and southernmost new harbours reach`)
w(`  -- each other at the distance this run measured, down the coast and not through the Andes.`)
{
  // the same coast: the southernmost new harbour within 90° of longitude of the northernmost, so
  // the control is a coastwise passage and not an ocean crossing to another growth's harbour
  const byLat = [...newPlaces].filter((p) => p.kind === 'HARBOUR').sort((a, b) => b.lat - a.lat)
  const sameCoast = byLat.filter((p) => Math.abs(p.lon - byLat[0].lon) < 90)
  if (sameCoast.length >= 2) {
    const n = sameCoast[0], sth = sameCoast[sameCoast.length - 1]
    const nm = reaches.get(n.code).get(sth.code)
    w(`  v_nm := (select (reaches->>${q(sth.code)})::numeric from public.sea_reaches where code = ${q(n.code)});`)
    w(`  if v_nm is null or abs(v_nm - ${nm.toFixed(1)}) > 0.5 then`)
    w(`    raise exception '${SHORT} self-assert FAIL: ${n.code}->${sth.code} is served at % nm; this run measured ${nm.toFixed(1)} nm', v_nm;`)
    w(`  end if;`)
  }
}
w()
...
// THE NEW PLACES FIRST, when there are any: a sea migration cut after a growth is FOR them.
const NEW_HEADLINE = newPlaces.length === 0 ? '' :
  `THE WATER REACHES THE NEW HARBOURS. ${newPlaces.length} place(s) the growth before this file inserted had no reach row and now carry a roadstead `
  + `(${newPlaces.map((p) => `${p.code} ${roads.get(p.code).nm.toFixed(2)}${roads.get(p.code).channel ? ' on ' + roads.get(p.code).channel : ''}`).join(' · ')} nm) `
  + `and a sailed distance to every other place; ${opened} cell(s) of water opened by the carve and named; ${moved.length} existing roadstead(s) move `
  + `and the existing pairs move a median ${medD.toFixed(2)} nm. `
