// Emits migration 0068 from data/fittings.json. Kept out of the migration for the reason
// scripts/build-demand-0066.mjs states: a harness written INTO a migration had to be taken out again.
...
for (const f of fittings) {
  if (f.recipe.length < 2 || f.recipe.length > 5) {
    throw new Error(`${f.code}: a recipe is 2-5 inputs (DESIGN 6.3), this one has ${f.recipe.length}`)
  }
  for (const r of f.recipe) {
    if (!goods.has(r.in)) throw new Error(`${f.code} wants "${r.in}", which data/goods.json does not define`)
  }
}

const kindRows = fittings
  .map((f) => `  (${q(f.code)}, ${q(f.name)}, ${q(f.slot)}, ${q(f.buys)}, ${q(f.spends)}, ${q(f.note)}, ${f.volume}, ${f.ws_tier})`)
  .join(',\n')

const recipeRows = fittings
  .flatMap((f) => f.recipe.map((r) => `  (${q(f.code)}, ${q(r.in)}, ${r.qty})`))
  .join(',\n')
...
fs.writeFileSync('supabase/migrations/20260818000068_a_fitting_is_made_not_found.sql', SQL)
console.log('wrote 0068:', fittings.length, 'fittings,', fittings.reduce((n, f) => n + f.recipe.length, 0), 'recipe lines')
