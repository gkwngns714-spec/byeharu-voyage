...
import { linesFor, receiptFor, stageLine, useManifest } from '../src/store/manifest'
...
test('the store: a new key drops the old lines, remove empties the key, settle spends the lines', () => {
  ...
  expect(useManifest.getState().lines).toEqual([])
  expect(useManifest.getState().key).toBeNull()
  // THE RECEIPT IS KEYED TO WHERE IT WAS SETTLED (PR #59 review MUST-FIX 2): another board reads
  // none, and a new line staged anywhere dismisses it — a new basket is a new story.
  expect(receiptFor(useManifest.getState(), 'f1', 'LIS')).toBe(receipt)
  expect(receiptFor(useManifest.getState(), 'f1', 'CAD')).toBeNull()
  expect(receiptFor(useManifest.getState(), 'f2', 'LIS')).toBeNull()
  s.stage('f1', 'LIS', buy('pepper', 80))
  expect(useManifest.getState().receipt).toBeNull()
  expect(useManifest.getState().lines).toEqual([buy('pepper', 80)])
  s.clear()
  s.settle('f1', 'LIS', receipt)
  s.dismissReceipt()
  expect(useManifest.getState().receipt).toBeNull()
